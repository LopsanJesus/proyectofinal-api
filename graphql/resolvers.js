...
const resolvers = {
  ...
  Query: {
    ...
    async getMyForest(_, args, { loggedUser }) {
      try {
        if (!loggedUser) throw new Error("You are not authenticated!");
        return models.ImportedTree.findAll({
    ...
    async getMyHistory(_, args, { loggedUser }) {
      try {
        ...
        const importedTrees = await models.ImportedTree.findAll({
          where: { userId: loggedUser.id },
        });
  ...
  Mutation: {
    ...
    async createTree(_, { name, sourceLang, targetLang }, { loggedUser }) {
      try {
        ...
        if (name.trim() === "") throw new Error("Tree name cannot be empty");
    ...
    async recordTest(
      _,
      { score, numberOfLeaves, names, hits, importedTreeId },
      { loggedUser }
    ) {
      try {
        if (!loggedUser) throw new Error("You are not authenticated!");
        ...
        names.map(async (element, index) => {
          ...
          const leafRecord = await models.LeafRecord.findOne({
            where: {
              [Op.and]: [
                { leafId: leaf.id },
                { importedTreeId: importedTreeId },
          ...
          if (!leafRecord) {
            await models.LeafRecord.create({
              attempts: 1,
  ...
  Tree: {
    ...
    async sourceLang(languageId) {
      return models.Language.findOne({
        where: { id: languageId.dataValues.sourceLang },
      });
    },
    async targetLang(languageId) {
      return models.Language.findOne({
        where: { id: languageId.dataValues.targetLang },
      });
    },
    async branches(tree) {
